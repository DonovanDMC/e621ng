class AvoidPostingVersion < ApplicationRecord
  ...
  delegate :artist, :artist_name, :artist_id, to: :avoid_posting
  ...
  module SearchMethods
    def artist_search(params)
      Artist.search(params.slice(:any_name_matches, :any_other_name_matches, :group_name).merge({ id: params[:artist_id], name: params[:artist_name] }))
    end

    def search(params)
      q = super
      artist_keys = %i[artist_name artist_id any_name_matches any_other_name_matches group_name]
