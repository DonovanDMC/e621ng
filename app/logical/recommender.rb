...
module Recommender
  ...
  def recommend_for_user(user, tags: nil, limit: 50)
    response = Faraday.new(Danbooru.config.faraday_options).get("#{Danbooru.config.recommender_server}/recommend/#{user.id}?limit=#{limit}")
  ...
  def recommend_for_post(post, tags: nil, limit: 50)
    response = Faraday.new(Danbooru.config.faraday_options).get("#{Danbooru.config.recommender_server}/similar/#{post.id}?limit=#{limit}")
    return [] unless response.success?
  ...
  # user_count: int
  def metrics
    response = Faraday.new(Danbooru.config.faraday_options).get("#{Danbooru.config.recommender_server}/metrics")
